  ...
  try {
    ...
    await sendgrid.send({
      ...
      html: `<p>Hello ${booking.firstName},</p>
             <p>Here is your secure deposit link:</p>
             <p><a href="${link}">💳 Pay Your Deposit</a></p>
             <p>Note: This is a hold of funds, not a charge. The hold will be released if not captured.</p>`,
    });
    ...
    await sendgrid.send({
      ...
      html: `<p>Deposit link sent to customer <b>${booking.email}</b>.</p>
             <p><a href="${link}">💳 Pay Link</a></p>`,
    });
  ...
  try {
    ...
    await sendgrid.send({
      ...
      html: `<p>Dear ${booking.firstName},</p>
             <p>A deposit hold of <b>£${(amount / 100).toFixed(2)}</b> has been placed for your booking <b>#${bookingID}</b>.</p>
             <p>This is an authorization only — no money has been taken yet. The hold may later be captured or released.</p>`,
    });
...
app.get("/test/email", async (req, res) => {
  try {
    await sendgrid.send({
      to: "kverhagen@mac.com",
      from: "Equine Transport UK <kverhagen@mac.com>",   // 👈 fixed
      subject: "Equine Transport UK | Test Email from Render Backend",
